...
$_lang['profile_msg'] = 'ここでは、このカスタマイズのプロファイルと、それに属する「セット」を管理します。<br />セットは、指定したページ（リソースの作成または更新）に適用される、カスタマイズをひとまとめにしたものです。They can also be restricted to certain Templates, or other field values on the Resource. Constraints for the Create Resource Sets will execute based on the parent of the newly created Resource&apos;s value.';
...
$_lang['set_constraint_desc'] = 'ここに指定した値と、「制約フィールド」に指定したフィールドがリソース上の値が、一致した場合のみ、このセットが適用されます。';
...
$_lang['set_msg'] = 'カスタマイズのセットの基本設定です。<br />カスタマイズの対象となる操作やテンプレートなど、このカスタマイズの適用条件を指定してください。';
...
$_lang['usergroup_remove_confirm'] = 'このプロファイルは、このユーザーグループのユーザーに適用されなくなります。削除してよろしいですか？';
