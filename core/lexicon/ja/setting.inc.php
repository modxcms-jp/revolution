...
$_lang['setting_access_category_enabled_desc'] = 'カテゴリーに対するアクセス権チェックの有無を指定します（コンテキストごと）。<p><strong>注：この設定を「いいえ」にした場合、全てのカテゴリーへのアクセス権設定は無視されます。</strong></p>';
$_lang['setting_access_context_enabled'] = 'コンテキストアクセス権チェック';
$_lang['setting_access_context_enabled_desc'] = '全コンテキストへのアクセス権チェックの有無を指定します。<p><strong>注：このオプションが無効の場合、全てのコンテキストアクセス権設定は無視されます。このシステム全体または"mgr"コンテキストにアクセスさせたくないとき以外はこの設定を無効にしないでください。</strong>';
$_lang['setting_access_resource_group_enabled'] = 'リソースグループのアクセス権をチェック';
$_lang['setting_access_resource_group_enabled_desc'] = '（コンテキストごとの）リソースグループのアクセス権チェックの有無を指定します。<p><strong>注：このオプションが無効の場合、全てのリソースグループアクセス権設定は無視されます。</strong>';
...
$_lang['setting_cache_resource_desc'] = '有効の場合、リソースのキャッシングはリソースによって設定されます。無効にした場合、グローバルに無効になります。';
...
$_lang['setting_manager_js_document_root_desc'] = 'サーバーがDOCUMENT_ROOTサーバー変数を提供しない場合、管理画面のJS/CSS圧縮に必要な指定を行います。必ず影響をよく理解した上で変更してください。';
$_lang['setting_manager_js_zlib_output_compression'] = '管理画面のJS/CSS圧縮 zlibの使用';
$_lang['setting_manager_js_zlib_output_compression_desc'] = '管理画面でのJS/CSS圧縮に、zlibの出力を使用するかを指定します。PHPの設定で zlib.output_compression を 1 に確実に設定できない限り、この設定を有効にしないでください。「いいえ」を推奨します。';
$_lang['setting_manager_lang_attribute'] = '管理画面の言語コード（HTMLとXML）';
$_lang['setting_manager_lang_attribute_desc'] = '管理画面に選択した言語設定に合う言語コードを指定してください。これにより、ブラウザが最適なコンテンツを提示します。';
...
$_lang['setting_manager_login_url_alternate_desc'] = '管理画面へのアクセスを要求するユーザーに提示する、ログイン用URL。管理画面にアクセスできるよう、"mgr"コンテキストへユーザーをログインする必要があります。';
$_lang['setting_manager_login_start'] = 'スタートページ';
$_lang['setting_manager_login_start_desc'] = 'トップページとして使用するドキュメントのIDを入力してください。<p><strong>注意: 既に存在するドキュメントのIDを入力してください。ドキュメントは公開されている必要があります</strong>';
...
$_lang['setting_site_status_desc'] = 'このサイトをウェブ上に公開するには「はい」（オンライン）を選択してください。<p>「いいえ」（メンテナンスモード）を選択した場合、訪問者には「メンテナンスモード用ページ」または「メンテナンスモード用メッセージ」が表示されます。';
...
$_lang['setting_unauthorized_page_desc'] = 'ユーザーがアクセスを許可されていないドキュメントを要求したとき表示するドキュメントのIDを入力してください。 <p><strong>注意: 実在するドキュメントのIDを指定してください。また公開中でしかもPublicなアクセスが可能であるものに限ります</strong>';
